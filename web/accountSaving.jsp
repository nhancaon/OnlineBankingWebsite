        ...
        <div class="my-16 py-8 px-20 rounded-xl bg-white">
            <div class="flex justify-between items-center">
                ...
                <button id="createAccountBtn" class="p-4 bg-gradient-to-r from-[#00bfae] to-[#0066ad] rounded-2xl outline-none 
                        focus:ring transform transition hover:scale-105 duration-300 ease-in-out" onclick="showCreateAccount()">Add Saving Account</button>
            </div>
            <div>
            ...
            <c:choose>
                <c:when test="${savingAccount != null}">
                    <div class="col-span-3 my-16 py-8 px-20 rounded-xl bg-white">
                    <label>Date Closed:</label>
                    <span class="text-[#2a6ebe]">${savingAccount.dateClosed}</span><br>
                    <label>Date Opened</label>
                    <span class="text-[#2a6ebe]">${savingAccount.dateOpened}</span><br>
                    <label>Interest Rate</label>
                    <span class="text-[#2a6ebe]">${savingAccount.interestRate}%</span><br>
                    <label>Saving Amount</label>
                    <span class="text-[#2a6ebe]">${savingAccount.savingAmount}</span><br>   
                    </div>      
                </c:when>
                <c:when test="${savingAccount == null}">
                   <p class="text-center mt-5">No saving found for the specified customer.</p>
                </c:when>
                <c:otherwise>
                    <p class="text-center mt-5">No saving found for the specified customer.</p>
                </c:otherwise>
